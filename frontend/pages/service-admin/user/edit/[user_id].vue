...
const { storeUser, getOneUserData, getAddressInfo } = user();
...
const update = async () => {
  ...
  error_message.value.innerHTML = await storeUser("update", fd);
...
const init = async () => {
  ...
  for (let i = 0; i < prefectureOptions.value?.length; i++) {
    if (
      prefectureOptions.value[i]["key"] ===
      user_data.value["prefecture"]?.toString()
    ) {
